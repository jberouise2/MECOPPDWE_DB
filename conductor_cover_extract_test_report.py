...
def conductor_cover_extract_test_result(dir, file, save):
    """
    Extract Test Result of Conductor Cover and importing it to a csv file.
    ...
    for sheet_names in excel_file.sheet_names:
        test_report_id_data = pd.read_excel(file_init, sheet_name=sheet_names, skiprows=7, usecols='W', nrows=3)
        sector_id_data = pd.read_excel(file_init, sheet_name=sheet_names, skiprows=7, usecols='G', nrows=3)
        date_received_data = pd.read_excel(file_init, sheet_name=sheet_names, skiprows=7, usecols='W', nrows=3)
        date_tested_id = pd.read_excel(file_init, sheet_name=sheet_names, skiprows=7, usecols='G', nrows=3)
        equipment_id = 'CONDUCTOR COVER'
        ...
        start_row = df[df.eq('Item No.').any(axis=1)].index[0]
    ...
    df_fin = df_fin[(df_fin["control no."].notna()) & (df_fin["control no."] != "NA")  ]

    #replacing column names
    df_fin.rename(columns={
                            "item no.": "item_no.",
                            "control no.": "control_no",
                            "voltage rating (kv)": "voltage_rating",
                            "test voltage (kv)": "test_voltage",
                            "results": "result"
                            }, inplace=True)
    ...
    print(df_fin)
    print(f"\nSuccessfully Saved File to {save_path}\n")

    
############################################################################    
    
def conductor_cover_to_postgres(dir, file, database):
    """
    ...
    df_fin.to_sql('conductor_cover', engine, if_exists='append', index=False)
